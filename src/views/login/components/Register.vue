...
<n-form :class="prefixCls" ref="formRef" :model="registerForm" :rules="rules" size="large" :show-label="false">
  <n-form-item path="username">
      <n-input @keydown.enter.prevent v-model:value="registerForm.username" :placeholder="$t('login.accountPlaceholder')" clearable>
  ...
  <n-form-item path="repeatPwd" first>
  ...
  <n-space :vertical="true" :size="18">
    <div class="w-full text-14px">
      <n-checkbox v-model:checked="agreement">我已经仔细阅读并接受</n-checkbox>
...
const rules: FormRules = {
  username: [formRules.createRequired(t('login.accountPlaceholder'), ['blur']),
    formRules.isExist('username')
  ],
  //phone通过phoneExtValidator验证处理
...
async function handleSubmit(e: MouseEvent){
  ...
  try{
    ...
    if(!agreement.value){
      window.$message?.warning(t('login.policyPlaceholder'))
      return
    }

    loading.value = true
    const {username, password:pwd, smsCode, phone:number, dialCode} = registerForm
...
<style lang="scss">
// @prefix: ~'@{namespace}-register-form';
// .@{prefix}{
$prefix: '#{$namespace}-register-form';
.#{$prefix}{
  .vue-tel-input{
