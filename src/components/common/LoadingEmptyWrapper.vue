  ...
  <div v-if="reloadFlag" class="relative">
    ...
    <div v-show="showPlaceholder" class="absolute-lt w-full h-full" :class="placeholderClass">
      ...
      <div v-show="isEmpty" class="absolute-center">
        <div class="relative">
          <SvgIcon name="empty-data" :class="iconClass" />
      ...
      <div v-show="!network" class="absolute-center">
        <div class="relative" :class="{ 'cursor-pointer': showNetworkReload }" @click="handleReload">
          <SvgIcon name="network-error" :class="iconClass" />
...
const props = withDefaults(defineProps<Props>(), {
  ...
  empty: false,
  loadingSize: 'medium',
  placeholderClass: 'bg-white dark:bg-transparent transition-background-color duration-300 ease-in-out',
  emptyDesc: i18n.global.t('loading.empty'),
...
const networkErrorDesc = computed(() =>
  props.showNetworkReload ? `${t('api.networkException')}, ${t('button.clickRetry')}` : t('api.networkException')
);
