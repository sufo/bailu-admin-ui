...
import { useGo } from '../common/usePage';
...
export function useMenuSearch(emit: EmitType) {
  ...
  let menuList: App.Menu[] = [];
  const routeStore = useAsyncRouteStore();
  const go = useGo();
  const handleSearch = useDebounceFn(search, 200);

  onBeforeMount(async () => {
    const list = routeStore.menus;
    //@ts-ignore
  ...
  function search(key: string) {
    ...
    //e is not assignable to type 'OnUpdateValue'.
    // e?.stopPropagation();
  ...
  function handlerSearchResult(filterMenu: App.Menu[], reg: RegExp, parent?: App.Menu) {
