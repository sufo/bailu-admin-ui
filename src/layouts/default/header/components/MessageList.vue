  ...
  <n-scrollbar class="max-h-360px">
    <n-list>
      <n-list-item
        ...
        :key="item.id"
        class="hover:bg-#f6f6f6 dark:hover:bg-transparent cursor-pointer"
        @click="handleRead(index)">
        <n-thing class="px-15px" :class="{ 'opacity-30': item.isRead }">
          <template #avatar>
            <n-avatar v-if="item.avatar" :src="item.avatar" />
            <svg-icon v-else-if="item.icon" class="text-34px text-primary" :name="item.icon" />
          </template>
          <template #header>
          ...
          <template #description>
            <n-ellipsis v-if="item.content" :line-clamp="2">
              {{ item.content }}
            </n-ellipsis>
            <p>{{ item.date }}</p>
          </template>
